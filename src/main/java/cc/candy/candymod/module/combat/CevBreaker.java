...
public class CevBreaker extends Module {
    ...
    public boolean headMode , based = false;
    ...
    public void doCB()
    {
        ...
        try {
            ...
            if(!builtTrap)
            {
                if(BlockUtil.getBlock(base) == Blocks.AIR && !based)
                {
                ...
                if(BlockUtil.getBlock(base.add(0 , 1 , 0)) == Blocks.AIR && !based)
                {
                    if(blockTimer.passedX(preDelay.getValue()))
                    {
                        setItem(obbySlot);
                ...
                if(BlockUtil.getBlock(headPos) == Blocks.AIR)
                {
                    ...
                    {
                        setItem(obbySlot);
                        placeBlock(headPos, packetPlace.getValue());
                        blockTimer = null;
                        builtTrap = true;
                    }
                }
                ...
                if(crystalTimer.passedX(crystalDelay.getValue()))
                {
                    ...
                    if (packetCrystal.getValue()) {
                        mc.player.connection.sendPacket(
                                new CPacketPlayerTryUseItemOnBlock(headPos, EnumFacing.DOWN, hand, 0, 0, 0));
                    } else {
                        mc.playerController.processRightClickBlock(mc.player, mc.world, headPos, EnumFacing.DOWN, new Vec3d(0, 0, 0), hand);
                    }
                    placedCrystal = true;
                }
            }
                    ...
                    if(!breaking) {
                        if(!instantBreak.getValue())
                        {
                            ...
                            mc.player.connection.sendPacket(new CPacketPlayerDigging(
                                    CPacketPlayerDigging.Action.START_DESTROY_BLOCK, headPos, EnumFacing.DOWN
                            ));
                            mc.player.connection.sendPacket(new CPacketPlayerDigging(
                                    CPacketPlayerDigging.Action.STOP_DESTROY_BLOCK, headPos, EnumFacing.DOWN
                            ));
                        }
                        else
                        {
            ...
            if(brokeBlock && !attackedCrystal)
            {
                ...
                {
                    BlockPos plannedCrystalPos = headPos.add(0 , 1 , 0);
                    ...
                    if(crystal == null)
                    {
                        ...
                        {
                            sendMessage("Cannot find crystal! retrying...");
                            reset();
                            return;
                        }
    ...
    public void onRender3D()
    {
        try {
            ...
            BlockPos headPos = new BlockPos(target.posX, target.posY , target.posZ).add(0 , 2, 0);
