...
public class Surround extends Module {
    public Surround(){
        super("Surround" , Categories.COMBAT , false , false);
    }
    ...
    private void placeBlock(BlockPos pos) {
        if (this.placements < this.blocksPerTick.getValue()) {
            ...
            int obbySlot = InventoryUtil.findHotbarBlock(Blocks.OBSERVER);
            int eChestSot = InventoryUtil.findHotbarBlock(Blocks.ENDER_CHEST);
            if (obbySlot == -1 && eChestSot == -1) {
                this.toggle();
            }
            isPlacing = true;
